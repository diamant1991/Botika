...
$(document).ready(function(){
	...
	$('a.close, #fade, #fade-2').live('click', function() { //закрытие по клику вне окна, т.е. по фону...
	  	$('#fade ,#fade-2, .popup_block').fadeOut(function() {
			$('#fade,#fade-2, a.close').remove();//исчезают и окно и слой по клику на кнопке  
	}); 
 ...
 $('.fade-2').click(function() {
		...
		//Добавляем полупрозрачный фон затемнения
		$('body').append('<div id="fade-2"></div>'); //div контейнер будет прописан перед тегом </body>.
		$('#fade-2').css({'filter' : 'alpha(opacity=70)'}).fadeIn(); //полупрозрачность слоя, фильтр для тупого IE
